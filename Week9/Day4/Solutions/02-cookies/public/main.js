...
function storeTheme(themeName) {
  // Your code here
  document.cookie = `theme-name=${themeName};max-age=10`;
}
...
function restoreTheme() {
  ...
  const themeName = getCookie("theme-name");
...
function restoreName() {
  // Your code here
  const displayName = getCookie("display-name");
...
const getCookie = (cookieName) => {
  ...
  const foundCookie = cookieList.find((cookie) => cookie.includes(cookieName));
